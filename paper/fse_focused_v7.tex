...
then put a second vendor's model (GPT-5.5, a cheaper, better writer) in the
check-writer's seat, and the bar failed a third time: shortening the checklist to save
...
and plain pass/fail task success, with no LLM grading anywhere. Pause attribution follows a written rule, with a hand-audited
...
the clean-overhead cap at 12\% in the pre-registration before any data existed, not a
quantity derived from a model; the frozen file records no rationale for the number,
which appears only as the KG3 clean-overhead threshold, and we disclose it as an
authors' judgment. Nothing downstream turns on the exact number. The verdict is invariant to the cap's value: the
...
and replanning nothing, discards none of it. It is not sunk pre-fault cost: the redesign
